 ...
 *
 * Modelled on Miniflux's entry filtering rules, minus the fields this reader cannot
 * support: Miniflux's EntryDate (non-regex syntax) and EntryCommentsURL (not carried
 * by the native gofeed bridge).
 *
 * The names deliberately drop Miniflux's "Entry" prefix - this reader calls these articles,
...
data class ArticleRule(
    ...
    fun matches(article: ParsedArticle): Boolean =
        when (field) {
            ...
            // content_text is never null for parsed feeds (it is KirinukiGoItem.plainContent,
